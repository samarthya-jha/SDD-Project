...
export class Agr extends React.Component {
  ...
  show( userInfo) {
    ...
    // let centreList;
    Axios.post("http://localhost:5000/manufacturer/agreements",data)
      .then((res) => {
        console.log(res);
        // this.handleSearch(res);
        this.show2(res.data.Live_Agreements,res.data.Upcoming_Agreements,res.data.Completed_Agreements,userInfo);
      })
      .catch((err) => {
          console.log("Axios", err.message);
      }); 
  }
    show2(live,upcoming,completed,userInfo){
    let len = live.length;
    let f1=(len>0);
    this.setState({f1:f1});
    let i;
      ...
      code.push(
        <div className="home__hero-row">
          ...
          <CardComponent1
            ...
            Vendor_Address={value1.Vendor_Address}
          />
          <CardComponent1
            flag={true}
            data={value2}
            ...
            Vendor_Address={value2.Vendor_Address}
          />
        </div>
      );
    }
  ...
  render() {
    // const { live,userInfo} = this.props;        /* tochange */
    ...
    return (
      ...
      <TnCModal
        ...
        head="Please post RFPs and finalize the RFPs to view them "
        show={ModalShow}
